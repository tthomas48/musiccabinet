...
public class JdbcUserRecommendedArtistsDao implements
		UserRecommendedArtistsDao, JdbcTemplateDao {

	private JdbcTemplate jdbcTemplate;

	@Override
	public void createUserRecommendedArtists(
			List<UserRecommendedArtists> userRecommendedArtists) {
	...
	private void batchInsert(List<RecommendedArtist> artists, LastFmUser user) {
		...
		batchUpdate.declareParameter(new SqlParameter("contextartist1_name",
				Types.VARCHAR));
		batchUpdate.declareParameter(new SqlParameter("contextartist2_name",
				Types.VARCHAR));

		for (int i = 0; i < artists.size(); i++) {
			RecommendedArtist rec = artists.get(i);
			batchUpdate.update(new Object[] { user.getLastFmUsername(),
					rec.getArtist().getName(), i,
					name(rec.getContextArtist1()),
					name(rec.getContextArtist2()) });
		}
		batchUpdate.flush();
	}
	...
	@Override
	public List<RecommendedArtist> getUserRecommendedArtists(
		...
		String sql = "select a.artist_name_capitalization,"
				...
				+ " where u.lastfm_user = upper(?)" + " order by rank";
		return jdbcTemplate.query(sql, new Object[] { lastFmUsername },
				new RowMapper<RecommendedArtist>() {
					@Override
					public RecommendedArtist mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						return new RecommendedArtist(rs.getString(1), rs
