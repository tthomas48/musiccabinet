...
public class JdbcTagDao implements TagDao, JdbcTemplateDao {
	...
	public void createTags(List<String> tags) {
		...
		batchUpdate
				.declareParameter(new SqlParameter("tag_name", Types.VARCHAR));
		batchUpdate
				.declareParameter(new SqlParameter("tag_name", Types.VARCHAR));
	...
	public void createTagCorrections(Map<String, String> tagCorrections) {
		...
		jdbcTemplate.update(
				sql,
				(Object[]) tagCorrections.keySet().toArray(
						new String[tagCorrections.size()]));
		jdbcTemplate.update(
				sql,
				(Object[]) tagCorrections.values().toArray(
						new String[tagCorrections.size()]));
		...
		BatchSqlUpdate batchUpdate = new BatchSqlUpdate(
				jdbcTemplate.getDataSource(), sql);
		batchUpdate.setBatchSize(1000);
		batchUpdate.declareParameter(new SqlParameter("t.tag_name",
				Types.VARCHAR));
		batchUpdate.declareParameter(new SqlParameter("tc.tag_name",
				Types.VARCHAR));
	...
	/*
	 * Returns a list of fairly popular and relevant tags that are used for
	 * artists found in library.
	 * 
	 * The tags chosen are the top 250 ordered by having most unique artists
	 * with a tag count of > 25, and a minimum of 5 artists being tagged with
	 * it.
	 */
	@Override
	public List<TagOccurrence> getAvailableTags() {
		...
		String sql = "select * from"
				...
				+ " inner join (select tag_id, count(tag_id) from "
				+ topTagsTable
				+ " att"
	...
	public List<String> getTopTags() {
		String sql = "select tag.tag_name from library.toptag tt"
	...
	/*
	 * Returns top tags (prerequisite), together with an equally distributed
	 * weighted system indicating popularity for each tag.
	 * 
	 * Right now, the popularity comes distributed in the interval 10-40, to
	 * support a tag cloud using the popularity as font size.
	 * 
	...
	public List<TagOccurrence> getTopTagsOccurrence() {
		String topTagsTable = settingsService.getArtistTopTagsTable();
		String sql = "select tag.tag_name, null, 10+ntile(30) over (order by pop.sum), true from library.toptag tt"
				+ " inner join music.tag tag on tt.tag_id = tag.id"
				+ " inner join (select coalesce(t.corrected_id, t.id) as tag_id, sum(tag_count)"
				+ "  from "
				+ topTagsTable
				+ " att inner join music.tag t on att.tag_id = t.id"
	...
	public List<Tag> getTagsWithoutTopArtists() {
		String topTagsTable = settingsService.getArtistTopTagsTable();
		String sql = "select t.id, t.tag_name from music.tag t"
				+ " left outer join (select tag_id, count(tag_id) from "
				+ topTagsTable
				+ " where tag_count > 25 group by tag_id) occ on t.id = occ.tag_id"
				+ " left outer join (select tag_id, sum(tag_count) from "
				+ topTagsTable
				+ " group by tag_id) pop on t.id = pop.tag_id"
				+ " left outer join (select tag_id from library.toptag) tt on t.id = tt.tag_id"
	...
	private void batchInsert(String tagName, List<Artist> artists) {
		...
		for (int i = 0; i < artists.size(); i++) {
			batchUpdate.update(new Object[] { tagName,
					artists.get(i).getName(), i });
		}
		batchUpdate.flush();
	}
