...
public class JdbcPlaylistGeneratorDao implements PlaylistGeneratorDao,
		JdbcTemplateDao {
	...
	@Override
	public List<? extends Uri> getTopTracksForArtist(Uri artistUri,
			int totalCount) {
	...
	/*
	 * Return a list of tracks, related to a given track. Pre-condition:
	 * music.trackrelation is populated for given track.
	 * 
	 * Strategy:
	 * 
	...
	public List<PlaylistItem> getPlaylistForTrack(Uri trackUri) {
		String sql = "select distinct on (t.id, tr.weight) a.id, lt.id"
				...
				+ " where tr.source_id = ?" + " order by tr.weight desc, t.id"
				+ " limit 25";
		return jdbcTemplate.query(sql, new Object[] { trackUri.getId() },
				new PlaylistItemRowMapper());
	}

	/*
	 * Return a list of tracks, related to a given artist. Pre-condition:
	 * music.artistrelation and music.artisttoptrack are populated.
	 * 
	 * Strategy:
	 * 
	 ...
	 * 
	 * Using this, pick the artists that will be eligible for being part of the
	 * final playlist, joined with their top tracks that we have a matching
	 * music file for.
	 * 
	 * Sort the resulting tracks by global track popularity + personal play
	 * count + some randomness, and then pick top (n) rows from each artist.
	 * 
	 * Sort the remaining tracks by artist relevance + a bit of randomness
	 * again, and pick top (m) rows that will go into the final playlist.
	 * 
	 * Intention: relevant artists appear more often (but not necessarily all
	 * the time). One single artist can never have more than (n) tracks in a
	 * given playlist. (unless the random() function very very unlikely makes
	 * rank() return double rows) Popular tracks for chosen artists appear more
	 * often, but less famous tracks might climb the ranks every now and then.
	 * 
	 * No assumptions can be made on order of returned tracks.
	 */
	@Override
	public List<PlaylistItem> getPlaylistForArtist(Uri artistUri,
			int artistCount, int totalCount) {
		Integer artistID = artistUri.getId();
		...
		String sql = "select artist_id, track_id from ("
				...
				+ "   inner join (select source_id, target_id, weight from music.artistrelation union all select ?, ? , 1) ar"
				+ "	    on ar.target_id = att.artist_id and ar.source_id = ? "
				+ "  ) ranked_tracks"
				+ "  where ranked_tracks.artist_rank <= ? "
				+ "  order by random() * ranked_tracks.artist_weight * ranked_tracks.artist_weight desc limit ?";
		return jdbcTemplate.query(sql, new Object[] { artistID, artistID,
				artistID, artistCount, totalCount },
				new PlaylistItemRowMapper());
	}
	 ...
	 * 
	 * Uses the same technique as getPlaylistForArtist(), but the ranking of
	 * artists is based on tag count fetched from last.fm.
	 * 
	 * Strategy: start by using tag correction to find max tag_count per tag.
	 * Then sum the individual max values per tag, for all tags queried for, to
	 * make up a tag relevance score per artist.
	 * 
	 * Use this tag relevance score per artist exactly the same way as artist
	 * relation score is used in getPlaylistForArtist().
	 */
	@Override
	public List<PlaylistItem> getPlaylistForTags(String[] tags,
			int artistCount, int totalCount) {
		...
		String sql = "select artist_id, track_id from ("
				...
				+ " 	select artist_id, max(tag_count) as tag_count from "
				+ topTagsTable
				+ " att"
	...
	@Override
	public List<PlaylistItem> getPlaylistForGroup(String lastFmGroup,
			int artistCount, int totalCount) {
		String sql = "select artist_id, track_id from ("
				...
				+ "  where ranked_tracks.artist_rank <= ? "
				+ "  order by random() * ranked_tracks.artist_weight * ranked_tracks.artist_weight desc limit ?";
		return jdbcTemplate.query(sql, new Object[] { lastFmGroup, artistCount,
				totalCount }, new PlaylistItemRowMapper());
	}
	 ...
	 */
	@Override
	public List<? extends Uri> getPlaylistForRelatedArtists(Uri artistUri,
			int artistCount, int totalCount) {
		String sql = "select track_id from ("
				...
				+ "   inner join music.artistrelation ar on ar.target_id = att.artist_id and ar.source_id = ? "
				+ "  ) ranked_tracks"
				+ "  where ranked_tracks.artist_rank <= ? "
				+ "  order by ranked_tracks.artist_weight desc, track_rank limit ? ";
		return jdbcTemplate.query(sql, new Object[] { artistUri.getId(),
				artistCount, (totalCount * artistCount) }, new UriRowMapper());
	}
	...
	@Override
	public void updateSearchIndex() { // TODO : evaluate time consumed +
										// optimize
		String sql = "select library.update_librarytoptracks(?,?)";
		jdbcTemplate.queryForInt(sql, new Object[] { minLength, maxLength });
	}
