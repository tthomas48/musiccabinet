...
public class JdbcWebserviceHistoryDao implements JdbcTemplateDao,
		WebserviceHistoryDao {
	...
	@Override
	public void blockWebserviceInvocation(Uri artistUri,
			WebserviceInvocation.Calltype callType) {
	...
	private void logWebserviceInvocation(WebserviceInvocation wi,
			Date invocationTime) {
		StringBuilder sql = new StringBuilder(
				"delete from library.webservice_history where calltype_id = "
						+ wi.getCallType().getDatabaseId());

		Integer artistId = null, trackId = null, albumId = null, userId = null, groupId = null, tagId = null;
		if (wi.getTrack() != null) {
			sql.append(" and track_id = "
					+ (trackId = musicDao.getTrackUri(wi.getTrack()).getId()));
		} else if (wi.getAlbum() != null) {
			sql.append(" and album_id = "
					+ (albumId = musicDao.getAlbumUri(wi.getAlbum()).getId()));
		} else if (wi.getArtist() != null) {
			sql.append(" and artist_id = "
					+ (artistId = musicDao.getArtistUri(wi.getArtist()).getId()));
		} else if (wi.getUser() != null) {
			sql.append(" and lastfmuser_id = "
					+ (userId = lastFmDao.getLastFmUserId(wi.getUser()
							.getLastFmUsername())));
		} else if (wi.getGroup() != null) {
			sql.append(" and lastfmgroup_id = "
					+ (groupId = lastFmDao.getLastFmGroupId(wi.getGroup()
							.getName())));
	 ...
	 * would be allowed.
	 * 
	 * Delegates decision to relevant method/query depending on type of
	 * invocation.
	 */
	@Override
	public boolean isWebserviceInvocationAllowed(WebserviceInvocation wi) {
		if (wi.getTrack() != null) {
			return isWebserviceInvocationAllowed(wi.getCallType(),
					wi.getTrack());
		} else if (wi.getAlbum() != null) {
			return isWebserviceInvocationAllowed(wi.getCallType(),
					wi.getAlbum());
	...
	protected boolean isWebserviceInvocationAllowed(Calltype callType,
			short page) {

		String sql = "select max(invocation_time) from library.webservice_history"
				+ " where calltype_id = ? " + " and page = ?";
		Timestamp lastInvocation = jdbcTemplate.queryForObject(sql,
				new Object[] { callType.getDatabaseId(), page },
				Timestamp.class);
	...
	protected boolean isWebserviceInvocationAllowed(Calltype callType,
		...
		String sql = "select max(invocation_time) from library.webservice_history h"
				...
				+ " where calltype_id = ? " + " and a.artist_name = upper(?)";
		Timestamp lastInvocation = jdbcTemplate.queryForObject(sql,
				new Object[] { callType.getDatabaseId(), artist.getName() },
				Timestamp.class);
		return isWebserviceInvocationAllowed(callType, lastInvocation);
	}
	...
	protected boolean isWebserviceInvocationAllowed(Calltype callType,
		...
		Timestamp lastInvocation = jdbcTemplate.queryForObject(sql,
				new Object[] { callType.getDatabaseId(),
	...
	protected boolean isWebserviceInvocationAllowed(Calltype callType,
		...
		String sql = "select max(invocation_time) from library.webservice_history h"
				+ " inner join music.lastfmuser u on h.lastfmuser_id = u.id"
				+ " where calltype_id = ? " + " and u.lastfm_user = upper(?)";
		Timestamp lastInvocation = jdbcTemplate.queryForObject(
				sql,
				new Object[] { callType.getDatabaseId(),
	...
	protected boolean isWebserviceInvocationAllowed(Calltype callType,
		...
		String sql = "select max(invocation_time) from library.webservice_history h"
				...
				+ " where calltype_id = ? " + " and g.group_name = upper(?)";
		Timestamp lastInvocation = jdbcTemplate.queryForObject(sql,
				new Object[] { callType.getDatabaseId(), group.getName() },
				Timestamp.class);
		return isWebserviceInvocationAllowed(callType, lastInvocation);
	}
	...
	protected boolean isWebserviceInvocationAllowed(Calltype callType, Tag tag) {
		...
		String sql = "select max(invocation_time) from library.webservice_history h"
				+ " where calltype_id = ? " + " and h.tag_id = ?";
		Timestamp lastInvocation = jdbcTemplate.queryForObject(sql,
				new Object[] { callType.getDatabaseId(), tag.getId() },
				Timestamp.class);
		return isWebserviceInvocationAllowed(callType, lastInvocation);
	}

	private boolean isWebserviceInvocationAllowed(Calltype callType,
		...
		if (lastInvocation == null) {
			...
			DateTime lastInvocationDateTime = new DateTime(
					lastInvocation.getTime());
			Days daysBetween = Days.daysBetween(lastInvocationDateTime,
					new DateTime());
	...
	/*
	 * Web services that are language dependant: artist.getInfo (5) tag.getInfo
	 * (not logged)
	 */
	@Override
	...
	/*
	 * Group artists in local library by last update time from last.fm, and
	 * return the 1/30th, but maximum 1000 artists, that were updated longest
	 * ago.
	 */
	@Override
	public List<String> getArtistNamesWithOldestInvocations(Calltype callType) {
		String sql = "select a.artist_name_capitalization from ("
				...
				+ "   from library.webservice_history where calltype_id = "
				...
				+ " and ntile.ntile = 1" + " order by artist_id limit 1000";

		return jdbcTemplate.queryForList(sql, String.class);
	}

	/*
	 * Return artists found in local library, who's never been looked up from
	 * last.fm.
	 * 
	 * Sets an upper limit of 3000 artists, meaning larger libraries will have
	 * to run the search index multiple times to get fully updated.
	 */
	@Override
